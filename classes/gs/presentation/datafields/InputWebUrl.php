...
class GS_InputWebUrl extends GS_DataField{
	
	function __construct($sId, $sLabel="WebSite: ", $bRequired=false){	
	...
	private function prepareFormField(){
		// Prefix
		$formF1 = new Sylar_FormInputText("p_".$this->getId(), "p_".$this->getId(), "www.");
		$formF1->setClass("sylarFrmField_address");
		$formF1->setMaxLen(100);
		$formF1->setSize(20);
		
		$formF1->setAttribute("onchange", "javascript:fieldWebUrlValidate('{$formF1->getId()}');");
				
		$inputSource = "http://".$formF1->getHtmlSource();
